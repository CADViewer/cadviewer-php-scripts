...
try {
	...
	if ($debug) {  // 2026-01-21
		$temp_log_name = rand();
		if ($fd_log = fopen($home_dir . "/php/logs/call-Api_Conversion_log_" . $temp_log_name . ".txt", "w")) {
	...
	try {
		...
		if ($conv_status)
			$temp_var = $json_request['converters'];
	} catch (Exception $e) {
	...
	if ($json_request['action'] == 'conversion' || $json_request['action'] == 'data_extraction' || $json_request['action'] == 'svg_js_creation_cvheadless' || $json_request['action'] == 'svg_js_creation' || $json_request['action'] == 'svg_creation' || $json_request['action'] == 'pdf_creation' || $json_request['action'] == 'svg_creation_sharepoint_REST') {
						...
						if ($converter == 'AutoXchange AX2011' && $parameters[$i]['paramName'] == 'f' && $json_request['action'] == 'svg_js_creation_cvheadless') {
							...
							if ($converter == 'AutoXchange AX2011' && $parameters[$i]['paramName'] != 'f' && $json_request['action'] == 'svg_js_creation_cvheadless') {
								...
								if ($parameters[$i]['paramName'] == 'xpath')
									$add_xpath = false;  // we are not using the config xpath, instead we use the one in the parameters
								if ($parameters[$i]['paramName'] == 'lpath')
									$add_lpath = false;  // we are not using the config xpath, instead we use the one in the parameters
								if ($parameters[$i]['paramName'] == 'fpath')
									$add_fpath = false;  // we are not using the config xpath, instead we use the one in the parameters
		...
		if ($json_request['action'] == 'svg_creation_sharepoint_REST') {
			...
			if ($contentusername != "") {
				if ($contenttype == 'file')
					cvjs_downloadSharepointFile($contentlocation, $fullPath, $Settings[$contentusername]['Url'], $Settings[$contentusername]['UserName'], $Settings[$contentusername]['Password'], $Settings[$contentusername]['RESTApiUrl'], $Settings[$contentusername]['DrawingUrl']);
				if ($contenttype == 'stream')
					cvjs_downloadSharepointFile($contentlocation, $fullPath, $Settings[$contentusername]['Url'], $Settings[$contentusername]['UserName'], $Settings[$contentusername]['Password'], $Settings[$contentusername]['RESTApiUrl'], $Settings[$contentusername]['DrawingUrl']);
			} else {
				if ($contenttype == 'file')
					cvjs_downloadSharepointFile($contentlocation, $fullPath, $Settings['Url'], $Settings['UserName'], $Settings['Password'], $Settings['RESTApiUrl'], $Settings['DrawingUrl']);
				if ($contenttype == 'stream')
					cvjs_downloadSharepointFile($contentstream, $fullPath, $Settings['Url'], $Settings['UserName'], $Settings['Password'], $Settings['RESTApiUrl'], $Settings['DrawingUrl']);
			}
...
//		$command_line = $command_line . " -i=" . $home_dir . '/' . $fullPath;
		...
		if ($json_request['action'] == 'pdf_creation') {
			...
			if (strrpos($contentlocation, '/') > 0)
				$fname = substr($contentlocation, strrpos($contentlocation, '/') + 1, strlen($contentlocation));
			if ($debug) {
				fwrite($fd_log, "pdf_creaton outfile \$fname:  $fname  \r\n");
			}
			if (strrpos($fname, '\\') > 0)
				$fname = substr($fname, strrpos($fname, '\\'), strlen($fname));
			if ($debug) {
				fwrite($fd_log, "pdf_creaton outfile \$fname:  $fname  \r\n");
			}
			if (strrpos($fname, '.') > 0)
		...
		if (strcmp($newenv, $saved) == 0) {
			...
			putenv("LD_LIBRARY_PATH=$ld_lib_path");
						...
						for ($i = 0; $i < $fsize; $i++) {
							...
							if ($continuecheckfiles) {
								...
								for ($j = 0; $j < $max; $j++) {
									...
									for ($k = 0; $k < $maxk; $k++) {
										if (
											$jarray['cachedfiles'][$i]['parameters'][$k]['paramName'] == $parameters[$j]['paramName']
...
//		echo "out= $out ";
		...
		$conversion_response = array(
			"completedAction" => $json_request['action'],
			"errorCode" => $return1,
			"converter" => $converter,
			"version" => $version,
			"userLabel" => $userlabel,
